...
function findScenarioOutlineExampleByLocation (gherkinDoc, sourceLocation) {
    var allExamples = gherkinDoc.feature.children.
...
export class CucumberEventListener extends EventEmitter {
    ...
    onTestStepStarted (testStepStartedEvent) {
        ...
        const doc = this.gherkinDocEvents.find(gde => gde.uri === uri).document
        const feature = doc.feature
        let scenario = doc.feature.children.find(child => child.location.line === sourceLocation.line)
        if(!scenario) {

            const allExamples = doc.feature.children.
                filter(child => child.type === 'Scenario Outline').
                map(scenarioOutline => scenarioOutline.examples).
                    map((exampleTable, exampleTableIndex) => exampleTable.tableBody.
                        map((example, exampleIndex) => Object.assign({},
                            example,
                            { exampleIndex, exampleTableIndex }
                        ))
                    ).
                    reduce((list, examples) => list.concat(examples), [])
            const example = allExamples.find(example => example.location.line === sourceLocation.line);
                scenario = {
